...
class _OrdersTabWidgetState extends State<OrdersTabWidget> {
  ...
  String _filterStatus =
      'all'; // 'all', 'pending', 'in_progress', 'ready', 'collected'
          ...
          child: _filteredOrders.isEmpty
              ...
              : RefreshIndicator(
                  ...
                  child: ListView.builder(
                    padding: EdgeInsets.fromLTRB(4.w, 4.w, 4.w, 10.h),
      ...
      child: Column(
        ...
        children: [
          ...
          Text(
            _orders.isEmpty
                ? 'No Orders Yet'
                : 'No ${Order.getStatusDisplay(_filterStatus)} Orders',
                  ...
                  children: [
                    Tooltip(
                      ...
                      child: OutlinedButton.icon(
                        onPressed: () => _generateReceipt(order),
                        icon: Icon(
                          Icons.receipt_long,
                          size: 18,
                          color: theme.colorScheme.primary,
  ...
  Future<void> _generateReceipt(Order order) async {
    try {
      final receipt = await _documentService.generateReceipt(order);

      await Share.shareXFiles(
        [XFile(receipt.path)],
        subject: 'TailorPro Receipt - ${order.orderNumber}',
        text: 'Receipt for ${order.orderTitle}',
      );
      ...
      try {
        ...
        if (authProvider.isAuthenticated) {
          try {
            ...
            debugPrint('WARNING: Order status cloud sync failed: $e');
          }
        }
        await _loadOrders();
        ...
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(
              content: Text(
                  'Order status updated to ${Order.getStatusDisplay(newStatus)}'),
              backgroundColor: Theme.of(context).colorScheme.primary,
              behavior: SnackBarBehavior.floating,
            ),
          );
...
class _OrderDetailsDialog extends StatelessWidget {
      ...
      content: SingleChildScrollView(
        child: Column(
          ...
          children: [
            ...
            const Divider(),
            _buildDetailRow(
                theme, 'Status', Order.getStatusDisplay(order.status)),
            if (order.stage != null)
              _buildDetailRow(theme, 'Stage', order.stage!),
            const Divider(),
            _buildDetailRow(theme, 'Total Amount',
                '₦${order.totalAmount.toStringAsFixed(0)}'),
            _buildDetailRow(theme, 'Paid Amount',
                '₦${order.paidAmount.toStringAsFixed(0)}'),
            _buildDetailRow(
              theme,
              'Balance',
            ...
            const Divider(),
            _buildOrderImagesSection(context, theme),
            if (order.notes != null && order.notes!.isNotEmpty) ...[
              const Divider(),
              Text(
                'Notes',
  ...
  Widget _buildOrderImagesSection(BuildContext context, ThemeData theme) {
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        Text(
        ...
        Wrap(
          ...
          runSpacing: 12,
          children: [
            _buildOrderImageSlot(
              context,
              theme,
              'Fabric Image',
              order.fabricImagePath,
              'No fabric image added',
            ),
            _buildOrderImageSlot(
              context,
              theme,
              'Style Image',
              order.styleImagePath,
              'No style image added',
            ),
          ],
        ),
      ],
  ...
  Widget _buildOrderImageSlot(
    ...
    final file = hasPath ? File(path) : null;

    return InkWell(
      onTap: hasPath ? () => _viewImage(context, label, path) : null,
      borderRadius: BorderRadius.circular(8),
      child: SizedBox(
        width: 120,
        child: Column(
  ...
  void _viewImage(BuildContext context, String title, String path) {
              ...
              Flexible(
                child: file.existsSync()
                    ...
                    : Padding(
                        ...
                        child: Text(
                          'This image file could not be found on this device.',
                          style: theme.textTheme.bodyMedium,
                          textAlign: TextAlign.center,
                        ),
                      ),
